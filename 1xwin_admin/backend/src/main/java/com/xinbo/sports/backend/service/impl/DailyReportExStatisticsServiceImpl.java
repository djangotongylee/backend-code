...
public class DailyReportExStatisticsServiceImpl {
    ...
    public List<ReportCenter.DailyReportStatisticsPlatCoinResBody> dailyReportStatisticsPlatCoin(ReportCenter.DailyReportStatisticsReqBody reqBody) {
        ...
        /** 排除测试账号 **/
        List<Integer> testUidList = userCache.getTestUidList();
        var boList = platformLeaderBoardServiceImpl.getDailyReportStatisticsPlatCoin(startTime, endTime, searchUidList, testUidList);
        if (Optional.ofNullable(boList).isPresent()) {
            var groupIdMap = boList.stream().collect(Collectors.groupingBy(ReportStatisticsPo::getGroupId));
            buildList.forEach(o -> {
                var dataList = groupIdMap.get(o.getGroupId());
                if (Optional.ofNullable(dataList).isPresent()) {
                    var coin = dataList.stream().map(ReportStatisticsPo::getCoin).reduce(BigDecimal::add).orElse(BigDecimal.ZERO);
                    var profit = dataList.stream().map(ReportStatisticsPo::getProfit).reduce(BigDecimal::add).orElse(BigDecimal.ZERO);
                    o.setBetCoin(coin);
                    o.setProfitCoin(profit);
                }
            });
        }
        return buildList;
     ...
     * @return
     */
    private List<CoinDeposit> getDepositStatisticsUIdCount(Integer startTime, Integer endTime, List<Integer> depStatusList, List<Integer> uidList, List<Integer> testUidList) {
        var sql = buildDepositWrapper(startTime, endTime, depStatusList, uidList, testUidList);
     ...
     * @return
     */
    private List<CoinWithdrawal> getWithdrawalStatisticsUIdCount(Integer startTime, Integer endTime, List<Integer> uidList, List<Integer> testUidList) {
        var sql = buildWithdrawalWrapper(startTime, endTime, uidList, testUidList);
        sql.select(Constant.ID);
        sql.groupBy(Constant.UID);
    ...
    public CapitalStatisticsListResBody dailyReportStatisticsSynthesis(ReportCenter.DailyReportStatisticsReqBody reqBody) {
        ...
        if (null != po && null != po.getCoinYXYK()) {
            po.setCoinYXYK(po.getCoinYXYK().negate());
        }
        return po;
    }
    ...
    private CapitalStatisticsListResBody returnDefaultValue() {
        return CapitalStatisticsListResBody.builder()
                .coinBet(BigDecimal.ZERO)
                .coinYXYK(BigDecimal.ZERO)
                .coinYJZC(BigDecimal.ZERO)
                .coinFSZC(BigDecimal.ZERO)
                .coinCZ(BigDecimal.ZERO)
                .coinTX(BigDecimal.ZERO)
                .coinXTTZ(BigDecimal.ZERO)
                .coinJLZC(BigDecimal.ZERO)
                .coinPTYK(BigDecimal.ZERO)
                .build();
    }
    ...
    public List<Integer> searchUserName2Uid(String searchUserName, Integer type) {
        ...
        // 查询代理下六级所有UID
        if (1 == type) {
            return userCache.getSupUid6ListByUid(searchUid);
        }
        // 查询全局的某一个UID
    ...
    public Integer getCacheUidByUserName(String userName) {
        Integer uid = null;
        if (StringUtils.isNotBlank(userName)) {
            var searchUserInfo = userCache.getUserInfoByUserName(userName);
            if (null != searchUserInfo) {
                uid = searchUserInfo.getUid();
            }
        }
        return uid;
    }

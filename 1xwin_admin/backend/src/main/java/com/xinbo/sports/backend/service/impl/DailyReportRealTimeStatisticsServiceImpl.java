...
public class DailyReportRealTimeStatisticsServiceImpl {
    ...
    private ReportCenter.DailyConversionReportListResBody generateDailyReport(Integer startTime, Integer endTime, List<Integer> testUidList) throws ExecutionException, InterruptedException {
        ...
        try {
            ...
            var thirdDepositCoinFuture = pool.submit(() -> getDepositStatisticsCoin(startTime, endTime, List.of(Constant.DEP_STATUS_THIRD), testUidList));
            var thirdDepositCountFuture = pool.submit(() -> getDepositStatisticsCount(startTime, endTime, List.of(Constant.DEP_STATUS_THIRD), testUidList));
            ...
            // 三存
            thirdDepositCoin = getCoin(thirdDepositCoinFuture.get());
            thirdDepositCount = getCount(thirdDepositCountFuture.get());
            // 提款记录
            ...
            if (Optional.ofNullable(firstDepositUidSetFuture.get()).isPresent()) {
                firstDepositUidList = firstDepositUidSetFuture.get().stream().map(CoinDeposit::getUid).collect(Collectors.toList());
            }
            if (Optional.ofNullable(userUidListFuture.get()).isPresent()) {
                var uidList = userUidListFuture.get().stream().map(User::getId).collect(Collectors.toList());
                // 注册人数
                registeredNum = uidList.size();
                if (registeredNum > 0) {
                    // 活跃人数
                    var activeNumFuture = pool.submit(() -> getUserLoginLogStatisticsUIdCount(startTime, endTime, uidList));
                    if (Optional.ofNullable(activeNumFuture.get()).isPresent()) {
                        activeNum = activeNumFuture.get().size();
                    }
                }
            }
            // 主键ID(格式yyyyMMdd)
            ...
            // 三存金额
            po.setThirdDepositCoin(thirdDepositCoin);
            // 三存笔数
            po.setThirdDepositCount(thirdDepositCount.intValue());
            // 存款金额
    ...
    private Map<String, Object> getDepositStatisticsCoin(Integer startTime, Integer endTime, List<Integer> depStatusList, List<Integer> testUidList) {
        ...
        sql.select("sum(pay_coin) as coin");
        return coinDepositServiceImpl.getMap(sql);
    }
     ...
     * @return UID列表
     */
    private List<CoinDeposit> getDepositStatisticsUIdList(Integer startTime, Integer endTime, List<Integer> depStatusList, List<Integer> testUidList) {
        var sql = buildDepositWrapper(startTime, endTime, depStatusList, testUidList);
        sql.select(Constant.UID);
        sql.groupBy(Constant.UID);
        return coinDepositServiceImpl.list(sql);
    }
    ...
    private Map<String, Object> getWithdrawalStatisticsCoin(Integer startTime, Integer endTime, List<Integer> testUidList) {
        ...
        sql.select("sum(coin) as coin");
        return coinWithdrawalServiceImpl.getMap(sql);
    }
     ...
     * @return 提款人数
     */
    private List<CoinWithdrawal> getWithdrawalStatisticsUIdList(Integer startTime, Integer endTime, List<Integer> testUidList) {
        var sql = buildWithdrawalWrapper(startTime, endTime, testUidList);
        sql.select(Constant.UID);
        sql.groupBy(Constant.UID);
        return coinWithdrawalServiceImpl.list(sql);
    }
    ...
    private BigDecimal getCoin(Map<String, Object> map) {
        return null == map ? BigDecimal.ZERO : (BigDecimal) map.get(COIN);
    }

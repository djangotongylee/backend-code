...
public class GameRecordServiceImpl implements IGameRecordService {
    ...
    /*字典表前缀*/
    public static final String CLASS_PATH = "com.xinbo.sports.backend.io.dto.";
    ...
    public List<GameListResBody> getGameListByGroupId(GameListReqBody dto) {
        ...
        for (var x : gameList) {
            var configJsonObject = parseObject(x.getModel());
            ...
            if (x.getGroupId() == 2) {
                ...
                gameNameMap = gameSlotList.stream().collect(Collectors.toMap(GameSlot::getId, (BaseEnum.LANG.ZH.getValue().equals(headerInfo.getLang()) ? GameSlot::getNameZh : GameSlot::getName)));
    ...
    public GrossResBody getGross(QueryGameRecordDto dto) {
        ...
        var configJsonObject = parseObject(gameLists.get(0).getModel());
        //获取gameId的映射字段
        var gameIdField = configJsonObject.getString(GAME_ID_FIELD);
    ...
    @Override
    public BetslipsCq9WaterMargin getCq9WaterMarginDetails(GameDetailsReqBody dto) {
        BetslipsCq9WaterMargin betslipsCq9WaterMargin = BeanConvertUtils.copyProperties(getGameDetails(dto, 406), BetslipsCq9WaterMargin::new);
        Map map = parseObject(toJSONString(betslipsCq9WaterMargin, SerializerFeature.WRITE_MAP_NULL_FEATURES), Map.class);
        return parseObject(toJSONString(toFormat(map)), BetslipsCq9WaterMargin.class);
    }

    @Override
    public BetslipsJokerGame getJokerGameDetails(GameDetailsReqBody dto) {
        BetslipsJokerGame betslipsJokerGame = BeanConvertUtils.copyProperties(getGameDetails(dto, 206), BetslipsJokerGame::new);
        Map map = parseObject(toJSONString(betslipsJokerGame, SerializerFeature.WRITE_MAP_NULL_FEATURES), Map.class);
        return parseObject(toJSONString(toFormat(map)), BetslipsJokerGame.class);
    }
    ...
    public ResPage<?> getBetsRecords(@Valid ReqPage<RecordParams.QueryGameRecordDto> platGameQueryDateDto) {
        ...
        BaseParams.HeaderInfo headerInfo = ThreadHeaderLocalData.HEADER_INFO_THREAD_LOCAL.get();//:en-英文, zh-cn-简体, zh-tw-繁体
        int i = BaseEnum.LANG.ZH.getValue().equals(headerInfo.getLang()) ? 0 : 1;

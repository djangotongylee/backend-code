...
import com.xinbo.sports.payment.utils.RsPayUtil;
...
@ApiSort(1)
@RequestMapping("/v1/rsPay")
@RequiredArgsConstructor(onConstructor = @__(@Autowired))
public class RsPayController {
    private final RsPayUtil rsPayUtil;

    @PostMapping(value = "/notifyUrl")
    @ApiOperation(value = "异步回调", notes = "在线支付异步回调")
    @ApiOperationSupport(author = "David", order = 2)
    public String notifyUrl(@Valid @RequestBody RsPayParams.NotifyUrlReqDto dto) {
        return rsPayUtil.notifyUrl(dto);
    }
    ...
    public String withdrawNotifyUrl(@Valid @RequestBody RsPayParams.DFNotifyUrlReqDto dto) {
        return rsPayUtil.withdrawNotifyUrl(dto);
    }
